...
its now over 1000 contributors, it has already more than 50 000 commits. As
you can see in the following image, the commits frequency is steady
throughout the years:

image:images/quarkus-code.png[]

And as you can see the release frequency follows the same line. Quarkus
releases a new feature release every month with a few micro releases in
between.

image:images/quarkus-tags.png[]

With such a frequent release, you might think it could be hard to keep up,
this is why Quarkus provides two ways in which you can consume releases:
latest/greatest and Long Term Support (LTS) releases. Both can be easily
upgraded with the build-in mechanism called Quarkus update. Then you can
simply run one of the following commands to update your Quarkus project to
...
# Maven plugin
./mvnw io.quarkus.platform:quarkus-maven-plugin:3.19.1:update -N
----
...
Quarkus community keeps supported for longer periods. You can check which
releases are LTS together with the proposed release planning at here
